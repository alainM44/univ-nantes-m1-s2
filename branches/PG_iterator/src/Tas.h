...
template<class T, class Compare = std::less<T>, class Alloc = std::allocator<T> >
class Tas<T, Compare, Alloc>::IteratorBfs
{
	...
	public:
		 ...
		 * \param[out] pos indice de l'élément à pointer dans le tas
		 */
		IteratorBfs(tas_type *t, int pos);
		...
		T operator*();
		/*!
		 * \fn IteratorBfs operator++()
		 * déplace d'une unité en "avant" selon DFS
		 */
		Iterator operator++();
		/*!
		 * \fn IteratorBfs operator==()
		 * \param I IteratorBfs à tester
		 *  Test l'égalité entre les deux  éléments pointés par deux itérateur
		 */
		bool operator==(const Iterator I);
...
/**** DEFINITION DES CORPS DES FONCTIONS de Tas::IteratorBfs*****/
template<class T, class Compare, class Alloc>
inline Tas<T, Compare, Alloc>::IteratorBfs::IteratorBfs() {
}
template<class T, class Compare, class Alloc>
inline Tas<T, Compare, Alloc>::IteratorBfs::IteratorBfs(tas_type* t) {
	t_ = t;
	pos_ = 0;
}

template<class T, class Compare, class Alloc>
inline Tas<T, Compare, Alloc>::IteratorBfs::IteratorBfs(tas_type*t, int pos) {
	t_ = t;
	pos_ = pos;
}
template<class T, class Compare, class Alloc>
inline typename Tas<T, Compare, Alloc>::IteratorBfs Tas<T, Compare, Alloc>::Iterator::operator ++() {
	pos_++;
	if(t_->indice_valide(pos_))
		return (*this);
	else
		return NULL;
}
...
template<class T, class Compare, class Alloc>
inline T Tas<T, Compare, Alloc>::IteratorBfs::operator*() {
	//TODO test
	return (t_->tas[pos_]); //TODOa vérifier
...
template<class T, class Compare, class Alloc>
inline bool Tas<T, Compare, Alloc>::IteratorBfs::operator==(const Iterator I) {
